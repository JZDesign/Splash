...
/// Grammar for the Kotlin language. Use this implementation when
/// highlighting Kotlin code. This is the default grammar.
...
private extension KotlinGrammar {
    static let keywords = ([
        ...
        "constructor", "delegate", "dynamic", "field", "file",
        "finally", "get", "import", "init", "param", "property",
        "receiver", "set", "setparam", "value", "where", "actual",
        "abstract", "annotation", "companion", "const", "crossinline",
        "data", "expect", "external", "final", "infix", "inline", "inner",
        "internal", "lateinit", "noinline", "open", "operator", "out", "override",
        "reified", "suspend", "tailrec", "vararg"
        
        
    ] as Set<String>).union(accessControlKeywords)

    static let accessControlKeywords: Set<String> = [
        "public", "internal", "fileprivate", "private", "sealed"
    ]

    static let declarationKeywords: Set<String> = [
        "class", "struct", "enum", "fun",
        "interface", "typealias", "import",
        "init"
    ]
    ...
    struct CallRule: SyntaxRule {
        ...
        init() {
            ...
            keywordsToAvoid.remove("suspend")
            keywordsToAvoid.remove("is")
            keywordsToAvoid.remove("reified")
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            if let previousToken = segment.tokens.previous {
                ...
                if !declarationKeywords.contains(segment.tokens.current) {
                    ...
                    if !segment.tokens.current.isAny(of: "self", "val", "var", "true", "false", "inout", "null", "try") {
